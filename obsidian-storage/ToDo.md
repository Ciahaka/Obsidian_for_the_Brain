...
Коллбэк в map типизируем как TasksType (если тип задан, то сама ide подскажет).
 ...
 !!! В метода map в обязательном порядке указываем атрибут key для возвращаемого элемента. Это поможет React работать эффективно при иттерации по массиву и при добавлении или удалении элементов из массива. Без атрибута key react может работать некорректно. 
