...
Div с сообщением можно разместить под кнопкой  добавления новой таски. Для того, чтобы это сообщение появлялось только в случае ошибки, используем инфиксный оператор &&. Данный оператор возвращает первый операнд с false.
![[Pasted image 20230330211219.png]]
 если error нет(false), то операнд прекращает работу
