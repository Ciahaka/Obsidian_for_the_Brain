...
### createRef
Метод, позволяющий обратиться к элементу виртуального DOM (получить ссылку на этот элемент). 
![[Pasted image 20230303190729.png]]
Сама ссылка является объектом к которому можно обратиться через dot  запись.
Чтобы свяать ссылку и нужный элемент, нужно назначить элементу атрибут ref с ссылкой внутри