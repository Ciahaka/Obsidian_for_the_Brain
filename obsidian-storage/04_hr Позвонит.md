...
Функция, которая оборачивает оригинальный компонент в компонент (контейнер)(принимает компонент и возвращает новый компонент). Используется для того, чтобы можно было переиспользовать логику. Для того, чтобы не дублировать код.
